...
function buildPrompt(input: {
  ...
  const parts: string[] = [
    ...
    "3. Para responder sobre 'falta entrar', dinero pendiente, o qué falta por depositarse de un medio de pago, DEBES FILTRAR SIEMPRE POR EL DÍA SELECCIONADO en la UI (`estado_actual.mes` y la fecha implícita que el usuario está viendo).",
    "   - REGLA DE ORO: Si el usuario pregunta '¿Cuánto falta por depositarse?', NO des el saldo global de toda la historia. Busca ÚNICAMENTE las ventas del día seleccionado (`snapshot_bancario_del_dia_seleccionado.business_date`) dentro de 'snapshot_bancario_actual_hoy.pending_items' filtrando por 'source_date'.",
    "   - Si no encuentras ningún item en `snapshot_bancario_actual_hoy.pending_items` que coincida con la fecha `business_date` seleccionada y el `channel` solicitado, significa que esas ventas YA SE DEPOSITARON en los días siguientes. En ese caso responde: '$0.00 (Ya ingresó todo al banco)'.",
